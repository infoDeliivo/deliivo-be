...
import { AlertCircle, Car, ChevronLeft, ChevronRight, Loader2, RefreshCw, Search } from 'lucide-react';
import { adminApi, AdminRide, Pagination } from '@/lib/api';
...
export default function AdminRidesPage() {
  ...
  useEffect(() => { loadRides(); }, [page, status, searchBy]);

  async function loadRides(nextSearch = search) {
    setLoading(true);
    setError('');
    try {
    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Search rides, inspect bookings, disputes, and operational state.</p>
        </div>
      ...
      <div className="rounded-2xl bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          ...
          <form onSubmit={(event) => { event.preventDefault(); setPage(1); loadRides(search); }} className="flex flex-col gap-2 lg:flex-row">
            <div className="relative">
              <Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400" />
              <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Ride, city, driver..." className="w-full rounded-xl border border-gray-200 py-2 pl-9 pr-3 text-sm focus:border-[#F97316] focus:outline-none lg:w-72" />
            </div>
      ...
      <div className="rounded-2xl bg-white shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-sm text-gray-400">No rides found.</div>
        ) : (
                  ...
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Link href={`/rides/${ride.id}`} className="font-semibold text-gray-900 hover:text-[#F97316]">
                    ...
                    <div className="mt-4 overflow-x-auto rounded-xl border border-gray-100">
                      <table className="w-full text-xs">
                        <thead className="bg-gray-50 text-gray-400">
                          <tr>
                            <th className="px-3 py-2 text-left font-medium">Booking ID</th>
                        ...
                        <tbody>
                          {ride.bookings.map((booking) => (
                            <tr key={booking.id} className="border-t border-gray-100">
                              <td className="px-3 py-2 font-mono text-gray-500">{booking.id.slice(0, 8)}</td>
                              <td className="px-3 py-2 text-gray-700">{booking.passenger?.name || booking.passenger?.email || booking.passengerId.slice(0, 8)}</td>
